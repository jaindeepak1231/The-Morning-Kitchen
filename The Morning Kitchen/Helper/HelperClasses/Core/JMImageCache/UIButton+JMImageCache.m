    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if(i) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [safeSelf setImage:i forState:UIControlStateNormal];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [safeSelf setImage:placeholderImage forState:UIControlStateNormal];
            ...
            [[JMImageCache sharedCache] imageForURL:url key:key completionBlock:^(UIImage *image) {
                if ([url isEqual:safeSelf.jm_imageURL]) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if(image) {
                            [safeSelf setImage:image forState:UIControlStateNormal];
                        } else {
                            [safeSelf setImage:placeholderImage forState:UIControlStateNormal];
                        }
