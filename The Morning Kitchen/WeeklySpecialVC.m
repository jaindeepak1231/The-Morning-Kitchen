...
@interface WeeklySpecialVC ()

@end

@implementation WeeklySpecialVC
...
- (void)viewDidLoad {
    ...
    if(appDelegate.isInternetAvailable) {
        ...
        [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(postDataOnWebserviceForWeeklySpecialData) userInfo:nil repeats:NO];
    }else{
...
-(void)postDataOnWebserviceForWeeklySpecialData {
    NSString *urlString = [NSString stringWithFormat:@"%@get_weekly_product.php", MainURL];
...
-(void)checkupdateFinished:(ASIFormDataRequest *)request {
    ...
    if (request.responseData) {
        NSError *error;
        arrWeeklyData = [NSJSONSerialization JSONObjectWithData:request.responseData options:NSJSONReadingMutableContainers|NSJSONReadingMutableLeaves error:&error];
        NSLog(@"%@",arrWeeklyData);
        [arrWeeklyData removeObjectAtIndex:0];
        NSLog(@"Result array :->%@",arrWeeklyData);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [[cell lblMenu]setText:[[arrWeeklyData objectAtIndex:indexPath.item]objectForKey:@"weekly_product_name"]];
    [[cell lblrs]setText:[NSString stringWithFormat:@"\u20B9 %@",[[arrWeeklyData objectAtIndex:indexPath.item]objectForKey:@"weekly_price"]]];
    
    NSString *strImage = [[arrWeeklyData objectAtIndex:indexPath.row]valueForKey:@"weekly_image"];
    ...
    NSString *strStock = [[arrWeeklyData objectAtIndex:indexPath.row]valueForKey:@"stock"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString*StrImage=[NSString stringWithFormat:@"%@",[[arrWeeklyData objectAtIndex:indexPath.item]objectForKey:@"weekly_image"]];
    NSString*StrName=[NSString stringWithFormat:@"%@",[[arrWeeklyData objectAtIndex:indexPath.item]objectForKey:@"weekly_product_name"]];
    NSString*StrPrice=[NSString stringWithFormat:@"%@",[[arrWeeklyData objectAtIndex:indexPath.item]objectForKey:@"weekly_price"]];
    ...
    NSString *strID = [[arrWeeklyData objectAtIndex:indexPath.row]valueForKey:@"id"];
