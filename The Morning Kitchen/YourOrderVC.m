...
@interface YourOrderVC ()

@end

@implementation YourOrderVC

- (void)viewDidLoad {
    [super viewDidLoad];
    arrYourOrderList = [[NSMutableArray alloc]init];
    ...
    if(appDelegate.isInternetAvailable) {
        ...
        [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(postDataOnWebserviceForYourOrderList) userInfo:nil repeats:NO];
    }else{
...
-(void)postDataOnWebserviceForYourOrderList {
    NSString *urlString = [NSString stringWithFormat:@"%@get_orderlist_by_userid.php?", MainURL];
...
-(void)checkupdateFinished:(ASIFormDataRequest *)request {
    ...
    if (request.responseData) {
        NSError *error;
        arrYourOrderList = [NSJSONSerialization JSONObjectWithData:request.responseData options:NSJSONReadingMutableContainers|NSJSONReadingMutableLeaves error:&error];
        NSLog(@"%@",arrYourOrderList);
            
        NSString *status = [NSString stringWithFormat:@"%@", [[arrYourOrderList objectAtIndex:0]valueForKey:@"status"]];
        if ([status isEqualToString:@"0"]) {
            return;
        }
        else {
            [arrYourOrderList removeObjectAtIndex:0];
            NSLog(@"Result array :->%@",arrYourOrderList);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString*strOrderNo = [NSString stringWithFormat:@"Order No : %@", [[arrYourOrderList objectAtIndex:indexPath.item]objectForKey:@"order_id"]];
    cell.lblOrderNo.text = strOrderNo;
    
    NSString*strDate = [NSString stringWithFormat:@"%@", [[arrYourOrderList objectAtIndex:indexPath.item]objectForKey:@"order_date"]];
    cell.lblDate.text = strDate;
    
    NSString*strType = [NSString stringWithFormat:@"%@", [[arrYourOrderList objectAtIndex:indexPath.item]objectForKey:@"order_type"]];
    cell.lblOrderType.text = strType;
    
    NSString*strPrice = [NSString stringWithFormat:@"RS: %@", [[arrYourOrderList objectAtIndex:indexPath.item]objectForKey:@"total_amount"]];
    cell.lblPrice.text = strPrice;
    
    NSString *strStatus = [[arrYourOrderList objectAtIndex:indexPath.item]objectForKey:@"order_status"];
    if ([strStatus isEqualToString:@"Pending"]) {
        cell.lblOrderStatus.text = @"Youe Order is Pending";
    }
    else {
