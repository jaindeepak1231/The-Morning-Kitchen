...
@interface ComboOfferVC ()

@end

@implementation ComboOfferVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    arrComboData = [[NSMutableArray alloc]init];
    ...
    if(appDelegate.isInternetAvailable) {
        ...
        [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(postDataOnWebserviceForComboOfferData) userInfo:nil repeats:NO];
    }else{
...
-(void)postDataOnWebserviceForComboOfferData {
    NSString *urlString = [NSString stringWithFormat:@"%@get_combo_offer.php", MainURL];
...
-(void)checkupdateFinished:(ASIFormDataRequest *)request {
    ...
    if (request.responseData) {
        NSError *error;
        arrComboData = [NSJSONSerialization JSONObjectWithData:request.responseData options:NSJSONReadingMutableContainers|NSJSONReadingMutableLeaves error:&error];
        NSLog(@"%@",arrComboData);
        [arrComboData removeObjectAtIndex:0];
        NSLog(@"Result array :->%@",arrComboData);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [[cell lblMenu]setText:[[arrComboData objectAtIndex:indexPath.item]objectForKey:@"combo_product_name"]];
    [[cell lblrs]setText:[NSString stringWithFormat:@"\u20B9 %@",[[arrComboData objectAtIndex:indexPath.item]objectForKey:@"combo_price"]]];
    
    NSString *strImage = [[arrComboData objectAtIndex:indexPath.row]valueForKey:@"combo_image"];
    ...
    NSString *strStock = [[arrComboData objectAtIndex:indexPath.row]valueForKey:@"stock"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString*StrImage=[NSString stringWithFormat:@"%@",[[arrComboData objectAtIndex:indexPath.item]objectForKey:@"combo_image"]];
    NSString*StrName=[NSString stringWithFormat:@"%@",[[arrComboData objectAtIndex:indexPath.item]objectForKey:@"combo_product_name"]];
    NSString*StrPrice=[NSString stringWithFormat:@"%@",[[arrComboData objectAtIndex:indexPath.item]objectForKey:@"combo_price"]];
    ...
    NSString *strID = [[arrComboData objectAtIndex:indexPath.row]valueForKey:@"id"];
